...
left join
DIM_LOCATION AS L on (L.IDLocation = T.IDLocation)
...
FACT_TRANSACTIONS as T
left join
DIM_MODEL as M on (M.IDModel = T.IDModel)
left join
DIM_MANUFACTURER as MF on (MF.IDManufacturer = M.IDManufacturer)
left join
DIM_LOCATION as L on (L.IDLocation = T.IDLocation)
where MF.Manufacturer_Name = 'Samsung'
Group by L.State
Having count(T.IDLocation)>0
order by L.State desc

/* Q.3	Show the number of transactions for each model per zip code per state. */

select
L.State, L.ZipCode, Count(T.IDCustomer) as Cnt
from
FACT_TRANSACTIONS as T
Right join
DIM_LOCATION as L on (L.IDLocation = T.IDLocation)
...
/* Q.5  Find out the average price for each model in the top5 manufacturers in terms of sales quantity and order by average price. */

select
TT.Model_Name, AVG(TT.Unit_price) as Average_Price
from
(select top 5
M.Model_Name, M.Unit_price
from
FACT_TRANSACTIONS AS T
left join
DIM_MODEL as M on (M.IDModel = T.IDModel)
left join
DIM_MANUFACTURER AS MF ON (MF.IDManufacturer = M.IDManufacturer)
order by T.Quantity desc) TT
Group by TT.Model_Name
order by Average_Price

/* Q.6 List the names of the customers and the average amount spent in 2009, where the average is higher than 500 */

select
C.Customer_Name, Avg(T.TotalPrice) as Average_Price
from
FACT_TRANSACTIONS AS T
left join
DIM_CUSTOMER as C on (C.IDCustomer = T.IDCustomer)
Where DATEPART(year,T.Date) = 2009
Group by C.Customer_Name
Having Avg(T.TotalPrice) > 500
order by Average_Price desc

/* Q.7	List if there is any model that was in the top 5 in terms of quantity, simultaneously in 2008, 2009 and 2010 */

with Data as
(select
M.Model_Name, T.Quantity,T.Date
from
FACT_TRANSACTIONS AS T
left join
DIM_MODEL as M on (M.IDModel = T.IDModel))
...
cross join
Data as D3
where DATEPART(year,D1.Date) = 2008 and DATEPART(year,D2.Date) = 2009 and DATEPART(year,D3.Date) = 2010
order by D1.Quantity desc

/* Q.8	Show the manufacturer with the 2nd top sales in the year of 2009 and the manufacturer with the 2nd top sales in the year of 2010. */

WITH  Rkg AS
(
SELECT MF.Manufacturer_Name,T.TotalPrice, T.Date, 
       DENSE_RANK() OVER (ORDER BY T.TotalPrice Desc) AS Rnk
from
FACT_TRANSACTIONS AS T
left join
DIM_MODEL as M on (M.IDModel = T.IDModel)
...
select
Manufacturer_Name, TotalPrice
from
Rkg
where DATEPART(year, Date) in (2009) and Rnk = 2
union all
select
Manufacturer_Name, TotalPrice
from
Rkg
where DATEPART(year, Date) in (2010) and Rnk = 2

/*Q.9	Show the manufacturers that sold cellphone in 2010 but didn’t in 2009.  */

select 
MF.Manufacturer_Name, datepart(year,T.Date) as Date_
from
FACT_TRANSACTIONS AS T
left join
DIM_MODEL as M on (M.IDModel = T.IDModel)
